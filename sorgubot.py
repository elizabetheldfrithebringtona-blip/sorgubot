...
BOT_TOKEN = "8871947071:AAF0uEk3wIushIdxP7kQsYB3jTNZe8N2D_Q"
...
class SorguStates(StatesGroup):
    ...
    adsoyad_ad = State()
    adsoyad_soyad = State()
    adsoyad_il = State() # HATA BURADAYDI: dsoyad_il yazıyordu, düzeltildi.
    adsoyad_ilce = State()
...
# Gönderim Seçenekleri (Yeni Özellik)
...
# ---------- YENİ: TXT / MESAJ SEÇİM İŞLEYİCİLERİ ----------
...
async def secim_mesaj_gonder(callback: types.CallbackQuery, state: FSMContext):
    ...
    if metin:
        if len(metin) > 4000: # Telegram karakter sınırı aşılırsa bölerek atar
...
async def secim_txt_gonder(callback: types.CallbackQuery, state: FSMContext):
    ...
    if metin:
        temiz_metin = re.sub(r'<[^>]+>', '', metin) # HTML etiketlerini txt için siler
